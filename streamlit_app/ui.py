...
def inject_css():
    ...
    .sponsor-card{
      background:#FFFFFF !important;          /* 전체 카드 흰색 */
      border:1px solid #e9e9e9 !important;
      border-radius:12px;
      padding:10px;
      height:110px;                            /* 동일 높이 유지 */
...
# /*항상 왼쪽 정렬하려면 위에 style에서 이거 넣으면 됨.*/
# .team-note{
#   text-align: left !important;
#   font-size: .9rem;
#   opacity: .85;
#   margin-top: .5rem;
#   margin-left: .25rem;  /* 살짝 들여쓰기 (원하면 제거) */
# }
...
# def render_competition_logo_topright():
#     """제목 오른쪽 상단 한 줄짜리 대회 로고"""
#     root = Path(__file__).resolve().parent.parent
#     src = _file_to_data_uri(root / "대회로고.png")
#     if src:
#         st.markdown(f'<div class="comp-logo"><img src="{src}" alt="대회 로고"/></div>',
#                     unsafe_allow_html=True)
...
def render_sponsor_strip(items=None, team_note: str | None = None):
    """
    오른쪽 패널용 스폰서 스트립.
    items: [("라벨", "파일명 또는 Path"), ...]
    team_note: 카드 밑에 표시할 문구(HTML 허용)
    """
    root = Path(__file__).resolve().parent.parent
    if items is None:
...
# ---- 지도 그리기 ----
def build_map(G, rects, path_ll, clicks, lat0, lng0, show_shadow: bool):
    m = folium.Map(location=[lat0, lng0], zoom_start=16, tiles=None)
    # 베이스 타일 2종 (Light / Dark)
    folium.TileLayer(
        "cartodbpositron",   # Light
        name="Light",        # 레이어 이름
        control=True,
        show=True            # 기본으로 켜둘 레이어
    ).add_to(m)
    ...
    # 그림자
    def add_shadow_rects_to_map():
        fg = folium.FeatureGroup(name="Shadows", show=True)
        for r in rects:
            pts_ll = [xy_to_ll(x, y, lat0, lng0) for (x, y) in r.corners()]
            folium.Polygon(pts_ll, color="#000000", weight=1, fill=True, fill_opacity=0.2).add_to(fg)
        fg.add_to(m)

    if show_shadow:
        add_shadow_rects_to_map()

    # 출발/도착 마커
    if len(clicks) >= 1:
        s_lat, s_lng = clicks[0]
        folium.Marker((s_lat, s_lng),
                      tooltip="출발지",
                      icon=folium.Icon(color="green", icon="play", prefix="fa")).add_to(m)
    if len(clicks) >= 2:
        e_lat, e_lng = clicks[1]
        folium.Marker((e_lat, e_lng),
                      tooltip="도착지",
...
def render_app():
    ...
    with hdr_r:
        render_competition_logo_topright(size_px=80)  # 60/64로 바꿔도 OK
    ...
    if "route" not in st.session_state:
        st.session_state.route = None  # {"nodes": [...], "path_ll": [...], "info": {...}, "src": int, "dst": int}
    ...
    with right:
        ...
        with c_t:
            t_val = st.time_input("시간(KST)", value=default_dt.time(), step=datetime.timedelta(minutes=10), key="time_kst")
        dt_local = ensure_kst(datetime.datetime.combine(d_val, t_val))

        c_w, c_shadow = st.columns([3, 2])
        with c_w:
        ...
        with c_btn1:
            do_calc = st.button("🧭 경로 계산", use_container_width=True, disabled=len(st.session_state.clicks) < 2)
        with c_btn2:
        ...
        with st.spinner("그림자 계산 중..."):
            rects, shaded_lookup = compute_shades_for_time(dt_local.replace(tzinfo=None).isoformat())
        ...
        if do_calc and (src is not None) and (dst is not None):
            ...
            st.session_state.route = {"nodes": path_nodes, "path_ll": path_ll, "info": info, "src": src, "dst": dst}

        # 선택/요약
        with st.expander("선택 상태", expanded=False):
            st.write("좌표(최대 2개):", st.session_state.clicks)
            if src is not None: st.write(f"출발 최근접 노드: **{src}**")
            if dst is not None: st.write(f"도착 최근접 노드: **{dst}**")
        ...
        if st.session_state.route is not None:
            ...
            m4.metric("Shade (%)", f"{info['shade_ratio']*100:.1f}") # 경로 전체 대비 그늘 비율
        else:
            if len(st.session_state.clicks) < 2:
    ...
    # ===== 왼쪽 지도 =====
    MAP_HEIGHT = 560
    MAP_TOP_OFFSET_PX = 8   # 👈 “진짜 조금만” 아래로 (원하면 6~12px로 조절)
    ...
    with left:
        ...
        m = build_map(
            G=G, rects=rects, path_ll=path_ll,
